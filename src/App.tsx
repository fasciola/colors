...
export default function App() {
  ...
  const generatePalette = async (colorHex: string = primaryColor, alignmentMode: string = mode, audience: string = targetAudience) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Service returned an unsuccessful code.");
      }
      ...
      console.error("Backend request failed, reverting to local mathematical solver:", error);
      // Mathematical rotational formulation fallback handles missing internet/API key
      alert("Local Fallback: Palette generated mathematically. Configure your Gemini Secret API key for tailored creative palettes!");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <header className="flex flex-col md:flex-row justify-between items-center bg-[#15171E]/95 border border-[#222530] rounded-2xl p-4 md:px-6 md:py-4 gap-4 shadow-xl mb-4">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="hidden lg:flex items-center gap-2 text-[10px] px-3 py-1.5 bg-[#1B1E28] border border-emerald-500/20 text-emerald-400 rounded-full font-mono">
              ...
              <span>Gemini Smart Models Operational</span>
            </div>
              ...
              <div>
                <div className="flex items-center justify-between border-b border-[#222530] pb-3 mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] uppercase tracking-widest text-slate-300 font-bold">Control Parameters</span>
                  </div>
